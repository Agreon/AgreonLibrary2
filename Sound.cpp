...
{ 
	...
	if(tmp == NULL) 
	{
            Log::write(std::string("ERROR: Could not load Sound file! Path: ") + path);
            return false; 
	}

	m_Chunks[name] = tmp;
        Log::write(std::string("Loaded Sound file successful. ")+name+" | "+path);
	return true;
    }
	...
	if(tmp == NULL) 
	{
            Log::write(std::string("ERROR: Could not load Music file! Path: ")+path);
            return false;
	}
	m_Music[name] = tmp;
        Log::write(std::string("Loaded Music file successful. ")+name+" | "+path);
	return true;
    }
	...
	{
            Log::write(std::string("ERROR: Could not play Sound with unkown ID! Name: ")+name);
            return 0;
	}
    }
